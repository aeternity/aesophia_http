...
```
curl -H "Content-Type: application/json" -d "{\"function\":\"set\",\"arguments\":[\"42\"],\"source\":\"$contract\",\"options\":{\"backend\":\"aevm\"}}" -X POST http://localhost:3080/encode-calldata
```
Returns:
```
...
```
curl -H "Content-Type: application/json" -d "{\"source\":\"$contract\",\"function\":\"init\", \"call-result\":\"ok\", \"call-value\":\"cb_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAACAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAKsxo1JY=\",\"options\":{\"backend\":\"aevm\"}}" -X POST http://localhost:3080/decode-call-result
```
